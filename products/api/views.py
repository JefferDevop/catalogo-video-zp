...
class ProductOEApiViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        # Optimizado para ofertas e inicio
        return Product.objects.filter(
            Q(offer=True) | Q(home=True)
        ).order_by('name_extend')

class CategoryProductApiViewSet(ModelViewSet):    
    serializer_class = CategoryProductSerializer
    # La clave está en select_related('product', 'category')
    queryset = CategoryProduct.objects.select_related('product', 'category').all().order_by('id')
    filter_backends = [DjangoFilterBackend]
